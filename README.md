...
이관 코드에서 동적 SQL을 만드는 부분을 분리하고, 최소한의 안전성 검증을 추가했습니다.

- `MigrationSqlBuilder`를 추가해 INSERT SQL 생성 책임을 분리했습니다.
- `MigrationSqlBuilderTest`를 추가해 정상 SQL 생성, 위험한 테이블명 거부, 빈 컬럼 목록 거부를 검증했습니다.
- 앞으로 더 확인하고 싶은 이관 기준은 [이관 검증 노트](docs/verification.md)에 정리했습니다.
